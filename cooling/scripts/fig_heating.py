...
for (i, delta) in enumerate(np.linspace(-20.0 * gamma, 0.0, 40)):
    my_ensemble = initial_state.copy()
    delta_v = np.sqrt(2.0 * kB * T_initial / mass)
    my_ensemble.v[:,:2] += np.random.normal(0, delta_v, my_ensemble.v[:,:2].shape)
